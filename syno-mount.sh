...
start(){
	...
	if [[ ! -f "/usr/bin/${list[rclone_config_index]}-rcloned" ]];then
		...
		echo -e "`red 已添加开机启动.`"
		echo
		echo -e "已创建服务，您可以通过如下命令来管理您的挂载服务。"
		echo
		echo -e "`red ${list[rclone_config_index]}-rcloned\ start`     启动服务"
		echo -e "`red ${list[rclone_config_index]}-rcloned\ stop`      停止服务"
		echo -e "`red ${list[rclone_config_index]}-rcloned\ restart`   重启服务"
		echo -e "`red ${list[rclone_config_index]}-rcloned\ status`    查看状态"
		echo -e "`red ${list[rclone_config_index]}-rcloned\ remove`    删除服务"
	else
